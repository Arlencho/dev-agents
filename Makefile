...
experience: ## Build Fleet Desk: data contract JSON then static site → site/experience/
...
test: ## Ground Truth unit tests (launchers, failover, routing, roster, dispatch lock, seat worktrees, autoplan fail-closed, vendor-auth, detached dispatch, orchestrator loop, critic verdict rule)
	...
	@echo ""
	@echo "== dispatch lock (one local dispatch per branch) =="
	@./tests/run-dispatch-lock-tests.sh
	@echo ""
	@echo "== seat worktrees (run-remote, per-branch lock end to end, seat sweep) =="
	@./tests/run-worktree-tests.sh
	@echo ""
	...
	@echo ""
	@echo "== plan parsing (task text survives verbatim) =="
	@./tests/run-plan-parse-tests.sh
	@echo ""
