...
#   wait <plan> [reason]               set (or, with no reason, clear) the
#                                      waiting reason: the runner writes it for
#                                      a plan whose AFTER header names a plan
#                                      that has not landed, and clears it itself
#   hold <reason>                      set the queue-wide hold the runner writes
#                                      while its memory guard is active
...
Usage: queue.sh <command> [args]   (every command prints the resulting order)
  ...
  unblock <plan>                     clear the blocked reason
  wait <plan> [reason]               set or clear the waiting reason (the
                                     runner's AFTER header bookkeeping)
  hold <reason>                      set the queue-wide hold (memory guard)
  release                            clear the hold
  list                               print the order and exit
...
case "$cmd" in
    add|rm|mv|start|settle|block|unblock|wait|hold|release|list) ;;
...
if cmd == "add":
    ...
    if len(args) < 1:
        die("usage: queue.sh wait <plan> [reason]", 2)
    idx = find(entries, args[0])
    if idx < 0:
        sys.stderr.write("queue.sh: %s is not in the queue\n" % args[0])
        rc = 4
    else:
        entries[idx]["waiting"] = scrub(" ".join(args[1:]))
        save(data)

elif cmd == "hold":
    if len(args) < 1:
        die("usage: queue.sh hold <reason>", 2)
    data["hold"] = scrub(" ".join(args)) or "held"
    save(data)
