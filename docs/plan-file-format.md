...
| `# ALLOW-FULL-SUITE` | `scripts/dispatch.sh` | Owner override for the fix-round gate. A fix round (this header, a round number above 1 in the header or a task, or a `-fixN` plan suffix) may VERIFY only the tests that cover the code touched plus the critic's failing tests; a task line carrying a full-suite VERIFY (`make test`, the whole tests directory runner, or the words "full suite", without the words "final round") stops the dispatch before it starts, and the stop message names the task line and the rule. This header lets the owner allow it anyway. |
| `# TIER: A` / `# TIER: B` / `# TIER: C` | `scripts/dispatch.sh` | The plan's risk tier (table: [`docs/risk-tiers.md`](risk-tiers.md)). The tier fixes the critic set, the round cap before a recorded merge-or-stop decision, and what counts as a blocking finding. A plan without a valid TIER header stops the dispatch before it starts, the stop message names the rule, and the dispatch banner names the tier. A plan that matches more than one Scope cell takes the higher-risk tier (A before B before C). |
| `# CONSTRAINTS: <rule>` | `scripts/dispatch.sh` | A rule that binds every seat of the plan, producers and critics alike. Repeatable: one rule per line. `scripts/dispatch.sh` puts the rules at the front of every task, numbered, so one wording reaches every seat instead of each task line retyping its own copy. Two gates keep the two places from drifting: a task line that repeats a rule verbatim stops the dispatch (say it once, in the header), and a plan whose seats did not receive the rules stops the dispatch as well, because silent injection failure would strip the rule from every seat at once. The dispatch banner names how many rules were added. Typical use: the house-style clause that 153 of the last 155 task lines were carrying by hand ([#115](https://github.com/Arlencho/dev-agents/issues/115)). |
| `# ALLOW-NO-TIER` | `scripts/dispatch.sh` | Owner override for the risk-tier gate: lets a plan dispatch without a TIER header. |
