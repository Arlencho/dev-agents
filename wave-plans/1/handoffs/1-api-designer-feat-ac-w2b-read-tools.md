# Handoff: feat/ac-w2b-read-tools, critic C1 fix (api-designer)

Commit `a0451064` on top of `0ed17f6a`, pushed to `origin/feat/ac-w2b-read-tools` (PR #2832). Scope of this pass: finding C1 only, from the critic comment on issue #2340 ("CRITIC W2B READ TOOLS"). B1 to B4, C2 and C3 are untouched and still open.

## Built

- `api.yaml`: `AirportResolveResponse.candidates` (optional array, omitted when the query is unambiguous) and a new `AirportCandidate` schema (`iata`, `label`, both required). One added response example, `ambiguous`, using the real Goa table values from `apps/api/internal/provider/ambiguous_place.go` (`GOA` "Genoa, Italy", `GOI` "Goa, India").
- `packages/api-client/types.ts`: regenerated, carries `candidates?: AirportCandidate[]` and the `AirportCandidate` interface.

## Decisions

- Shape mirrors the handler exactly: `Candidates []AirportCandidate json:"candidates,omitempty"` with `IATA`/`Label` non-omitempty, so `candidates` is optional on the response and `iata`/`label` are required on the item.
- `candidates` was NOT added to `required[]`, and `resolved`/`iata` semantics are unchanged, per the handler comment (homepage gate ignores the field).
- PRD coverage: `docs/prd/pages/assistant-channel.md` section 3.3 line 136 already describes `candidates` for `needs_clarification`, so no PRD change was needed (rule 2 satisfied).
- `apps/web/lib/generated-types.ts` was synced by `make generate` but restored and not committed: it is outside api-designer scope and the pre-commit hook rejects it. No CI gate compares the two copies (grep over Makefile, workflows, scripts: no hit). web-frontend or the orchestrator should run `make generate` on this branch and commit that file.
- No `Co-Authored-By` trailer (OLY-4, house style).

## Do not repeat

- `grep --include=*.go` fails under zsh without quoting the glob; quote it or use `-r` with a path.
- The endpoint key in `api.yaml` is `/airports/resolve:` with no `/api/v1` prefix; anchor searches on that.

## Evidence

```
npx @redocly/cli lint api.yaml   -> valid, 12 warnings (pre-existing), LINT_EXIT=0
make generate                    -> Generated packages/api-client/types.ts, GENERATE_EXIT=0
make check-api-spec              -> OK: 88 Go routes <-> 83 OpenAPI paths in sync, CHECK_API_SPEC_EXIT=0
dash scan on added api.yaml lines -> no match
git commit                       -> COMMIT_EXIT=0, a0451064, 2 files changed, 60 insertions
```

## Next hint

C2 (handler calls `provider.*` directly, should go through `AgentService`) belongs to go-backend and does not change the wire shape declared here.

---

# Prior handoff (previous session, kept verbatim)
