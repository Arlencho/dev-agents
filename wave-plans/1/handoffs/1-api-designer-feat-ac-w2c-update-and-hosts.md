...
# api-designer: critic item 7 on #2340 (PR #2833)

## Built
- `api.yaml`: `ResultSession.revision` (int32, required, present on every read); `PatchResultSessionRequest.expected_revision` (int32, optional for web callers, required on the assistant scope, refused 400 without it); `PATCH /result-sessions/{id}` gains a `409` whose `data` is the current `ResultSession` via `ResultSessionEnvelope`, and its `400` description names the missing `expected_revision` case on the assistant scope.
- `packages/api-client/types.ts`: regenerated.
- Commit `fae97478` on `feat/ac-w2c-update-and-hosts`, pushed.

## Decisions
- 409 reuses `ResultSessionEnvelope` (both `data` and `error` set) because the handler writes `APIResponse[*service.ResultSession]{Data: stale.Current, Error: ...}`; no new schema.
- `revision` is `required` on `ResultSession` because the Go struct has no `omitempty` and the service emits it on both projections.
- No PRD stop: `assistant-channel.md` section 3.3.7 and `03-trip-folio.md` line 74 already describe the field and the conflict.
- `apps/web/lib/generated-types.ts` is re-synced by `make generate` but sits outside the api-designer allowlist (`api.yaml packages/`), so it was reverted in the working tree and left for web-frontend's next `make generate`. No CI workflow checks that copy for drift.

## Do not repeat
- The PATCH operation is `operationId: updateResultSession`, not `patchResultSession`.
- The commit guardrail rejects the word pattern "Generated ... with" even when it refers to `make generate`; say "refreshed via" instead.
- Under zsh use `$?` after an unpiped command (or `$pipestatus[1]`), not `${PIPESTATUS[0]}`, to capture exit codes.

## Evidence
- `npx @redocly/cli lint api.yaml`: exit 0, 12 warnings, none on result-session schemas or path (all pre-existing).
- `make generate`: exit 0.
- `make check-api-spec`: exit 0, "88 Go routes, 83 OpenAPI paths in sync".
- Banned-dash scan over added lines: 0 hits.
