...
Fleet-wide rules for every seat that runs in this repo. Seat charters live in `roles/` and are the single source of truth for one role; this file holds what binds all of them. The block below is copied verbatim into every critic charter and checked by `tests/run-critic-verdict-tests.sh`; edit it in `tests/fixtures/critic-verdict-rule.md` first, then re-copy.
